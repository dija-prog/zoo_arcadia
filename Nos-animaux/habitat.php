...
$collection = $client->zoo_arcadia->animals;

// $animalName = $_GET['animal_nom'];

// if(isset($_GET[ $animalName])){
//   $animals = $collection->findOne(['animal_nom' =>  $animalName]);
//   if ($animals) {
...
//     );
//   } else {
//     $collection->insertOne(['animal_nom' => $animalName, 'views' => 1]);
//   }

//   echo "Consultation de " .  $animalName .  "mise à jour avec succés";
// }
...
<body>
  ...
  <main>
    ...
    <!-- récupere les views des animaux -->
    <?php
    if (isset($_GET[$animalName])) {
      $animals = $collection->findOne(['animal_nom' =>  $animalName]);
      $animalName = $_GET['animal_nom'];
